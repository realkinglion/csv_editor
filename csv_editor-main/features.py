...
class ColumnMergeManager:
    ...
    def _get_merge_separator(self):
        """結合時のセパレータを取得"""
        dialog = MergeSeparatorDialog(self.app.parent, self.app.theme, is_column_merge=True)
        self.app.wait_window(dialog)
        return dialog.result
...
# ▼▼▼ レベル4：非同期処理マネージャー（新規追加） ▼▼▼
class AsyncDataManager:
    """非同期データ管理（UI統合版）"""

    def __init__(self, app):
        self.app = app
        # UIが応答不能になるのを防ぐため、ワーカースレッドは1つに制限
        ...
        while True:
            try:
                ...
                if task['type'] == 'fetch_data':
                    self._do_fetch_data(task['indices'])

            except Exception as e:
                print(f"AsyncDataManager worker error: {e}")

    def _do_fetch_data(self, indices_to_fetch):
        """バックグラウンドでデータを取得する"""
        try:
            if self.app.db_backend:
                df = self.app.db_backend.get_rows_by_ids(indices_to_fetch)
            elif self.app.lazy_loader:
                # lazy_loaderはstart/endで受け取るため、連続したインデックスか判定
                # 簡単のため、ここでは最初のインデックスから連続で取得
                start = indices_to_fetch[0]
                end = indices_to_fetch[-1] + 1
                df = self.app.lazy_loader.get_rows(start, end)
            else:
                return # 通常モードでは不要

            if not df.empty and self.app.winfo_exists():
                # UIスレッドに結果を渡す
                self.app.after(0, self.app._update_rows_from_df, df)
        except Exception as e:
            print(f"Error fetching data in background: {e}")
