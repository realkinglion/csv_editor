...
import re
import pandas as pd
import config # ENABLE_ANIMATIONS を参照するためにインポート
...
#==============================================================================
# フェーズ3-B: RippleButton クラス (新規追加)
#==============================================================================
class RippleButton(ttk.Button):
    """クリック時にリップルエフェクトを持つボタン"""
    def __init__(self, parent, **kwargs):
        # commandをkwargsから分離し、super().__init__に渡さない
        self._command = kwargs.pop("command", None)
        super().__init__(parent, **kwargs)
        
        self.bind("<Button-1>", self._on_click, add='+')
        
        # CsvEditorAppのインスタンスからテーマを取得
        self.theme = None
        widget = self
        while widget.master:
    ...
    def _on_click(self, event):
        """リップルアニメーションの実行とコマンドの呼び出し"""
        # ボタンが無効状態なら何もしない
        if self.cget('state') == tk.DISABLED:
            return

        # アニメーションの実行
        if config.ENABLE_ANIMATIONS and self.theme:
            canvas = tk.Canvas(self, highlightthickness=0, background=self.theme.PRIMARY_HOVER)
    ...
    def _animate_ripple(self, canvas, x, y, radius=0):
        ...
        canvas.create_oval(
            ...
            fill=self.theme.PRIMARY_ACTIVE, outline="", tags="ripple"
        )
        
        # Canvasの透明度を徐々に上げることでフェードアウト効果を出す
        # TkinterのCanvasはオブジェクトごとの透明度をサポートしないため、
        # Canvas自体を透明にしていく
        alpha = max(0.0, 1.0 - (radius / max_radius))
        # この機能はプラットフォーム依存性が高いため、今回は実装しない
        
        self.after(15, lambda: self._animate_ripple(canvas, x, y, radius + step))
...
class CellSelectionMixin:
    ...
    def _show_context_menu(self, event, context, **kwargs):
        ...
        self.context_menu.add_command(label="✂️ 切り取り", accelerator="Ctrl+X", command=lambda: self._cut(), state=state_selected)
        self.context_menu.add_command(label="📋 コピー", accelerator="Ctrl+C", command=lambda: self._copy(), state=state_selected)
        self.context_menu.add_command(label="📎 貼り付け", accelerator="Ctrl+V", command=lambda: self._paste(), state=state_paste)
        ...
        if context == 'header':
            ...
            self.context_menu.add_command(label="🔄 ソートをクリア", command=lambda: self._clear_sort())
            self.context_menu.add_separator()
            
            self.context_menu.add_command(label=f"➡️ この右に列を挿入...", command=lambda: self.add_column(after_column_name=col_name))
            self.context_menu.add_separator()
            
            self.context_menu.add_command(label="B 列をコピー", command=lambda: self.copy_selected_column())
            paste_state = tk.NORMAL if self.column_clipboard is not None else tk.DISABLED
            self.context_menu.add_command(label="B 列に貼り付け", command=lambda: self.paste_to_selected_column(), state=paste_state)
            self.context_menu.add_separator()
            ...
            merge_menu.add_command(label="右と結合 →", command=lambda: self._merge_right())
            merge_menu.add_command(label="← 左と結合", command=lambda: self._merge_left())
            self.context_menu.add_separator()
            self.context_menu.add_command(label="🗑️ 選択行を削除", command=lambda: self.delete_selected_rows(), foreground="red")
    ...
    def _add_dynamic_menu_items(self):
        ...
        if self._detect_pattern():
            if self.context_menu.index('end') is not None:
                self.context_menu.add_separator()
            self.context_menu.add_command(label="✨ パターンで自動入力...", command=lambda: self._smart_fill())
    ...
    def _select_cell(self, item, col_id, col_name):
        ...
        if (item, col_name) not in self.selected_cells:
            ...
            self._animate_cell_selection(item, col_id) 
        self._update_action_button_states()
        self._update_status_bar()
        self.show_context_hint('cell_selected')
        
    def _animate_cell_selection(self, item, col_id):
        """セル選択時のアニメーション効果（条件付き）"""
        if not self._should_enable_animations() or not self.tree.exists(item):
            return
        
        original_color = self.theme.CELL_SELECT_START
        highlight_color = self.theme.PRIMARY_HOVER
        
        tags = self.tree.item(item, 'tags')
        tag_to_animate = 'selected_cell_oddrow' if 'oddrow' in tags else 'selected_cell_evenrow'
        
        def pulse(step):
            if not self.tree.exists(item) or step > 3:
                self.tree.tag_configure(tag_to_animate, background=original_color)
                return
            
            color = highlight_color if step % 2 == 0 else original_color
            self.tree.tag_configure(tag_to_animate, background=color)
            self.after(60, lambda: pulse(step + 1))
    ...
    def _get_row_data_by_iid(self, iid):
        """行ID(iid)から行データを取得する（遅延読み込み対応）"""
        try:
            row_index = int(iid)
            if hasattr(self, 'lazy_loader') and self.lazy_loader:
                df_row = self.lazy_loader.get_rows(row_index, row_index + 1)
                if not df_row.empty:
...
class CardViewNavigationMixin:
    ...
    def _bind_card_navigation(self):
        for key, handler in self.card_nav_bindings.items():
            self.parent.bind_all(key, handler)
...
class InlineCellEditor:
    ...
    def start_edit(self, event):
        if self.app.lazy_loader or self.app.db_backend:
            messagebox.showinfo("制限", "読み取り専用モードではインライン編集はできません。")
            return
